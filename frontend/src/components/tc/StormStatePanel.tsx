...
export function StormStatePanel({ data }: Props) {
  ...
  return (
    <section className="flex h-full flex-col border border-hairline bg-panel shadow-md">
      ...
      <div className="border-b border-[#1C4D8D] px-5 py-5" style={{ borderLeft: `3px solid ${color}` }}>
        ...
        <div className="mt-1 flex items-end gap-2">
          ...
          <span className="readout mb-1 text-lg text-[#BDE8F5] font-semibold">kt</span>
          <span
        ...
        <p className="mt-2 text-sm text-[#EEF8FC] font-medium">{data.category}</p>
        <p className="readout mt-2 text-xs text-[#8CB8E8]">
          {formatCoords(data.coordinates.lat, data.coordinates.lon)} · {data.storm_name}
        </p>
      </div>

      <div className="px-5 py-3">
        <h3 className="mb-2 text-xs font-medium text-[#8CB8E8] uppercase tracking-wider font-mono">
        ...
        <dl>
          {envRows(data.environmental).map((row) => (
            <div
              key={row.k}
              className="flex items-baseline justify-between gap-3 border-b border-[#1C4D8D]/40 py-2 last:border-0"
            >
              <dt className="text-[13px] text-[#EEF8FC]/90 leading-tight">{row.k}</dt>
              <dd className="flex items-baseline gap-3">
                <span className="readout text-[13px] whitespace-nowrap text-[#BDE8F5] font-bold">{row.v}</span>
                <span
                  className="w-[7.5rem] text-right text-[11px] font-medium"

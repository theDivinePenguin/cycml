...
export function buildForecastFromRealData(
  ...
  const lifecycle = timesteps.map((t, idx) => ({
    ...
    observed_kt: Math.round(t.vmax_curr * 10) / 10,
    actual_plus_24h: Math.round(t.vmax_plus_24h * 10) / 10,
    pred_6h: Math.round(t.predicted_plus_6h * 10) / 10,
    pred_12h: Math.round(t.predicted_plus_12h * 10) / 10,
    pred_24h: Math.round(t.predicted_plus_24h * 10) / 10,
  ...
  return {
    ...
    current_wind_kt: Math.round(curr.vmax_curr * 10) / 10,
    ...
    forecast: {
      "+6h": Math.round(curr.predicted_plus_6h * 10) / 10,
      "+12h": Math.round(curr.predicted_plus_12h * 10) / 10,
      "+24h": Math.round(curr.predicted_plus_24h * 10) / 10,
    },
    timeline,
    actual_outcome_kt: Math.round(curr.vmax_plus_24h * 10) / 10,
    lifecycle,
    model_id: modelId,

"""Export cyclone lifecycles and AI predictions for the test clone website using the best model (exp2_delta_moderate)."""
import argparse
import json
import re
...
def clean_ts(val):
    ...
    return int(match.group(1)) if match else int(val)


def export_test_clone_data(
    pred_csv_path: str = "experiments/ri_target_loss/results/exp2_delta_moderate/test_predictions.csv",
    ...
    if pred_df is not None and "target_t_timestamp" in pred_df.columns:
        pred_df["clean_ts"] = pred_df["target_t_timestamp"].apply(clean_ts)
    ...
    for storm_meta in storms_catalog:
        ...
        for step_idx, row in sub_df.iterrows():
            ...
            if pred_row is not None:
                ...
                # Direct raw reconstructed outputs from Best Delta Model
                # recon_plus_Xh = v_curr + pred_delta_Xh
                ...
                # Fallback to linear persistence
    ...
    for p in out_json_paths:
        ...
        print(f"Exported {len(export_payload)} storm lifecycles to {path}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pred-csv",
        type=str,
        default="experiments/ri_target_loss/results/exp2_delta_moderate/test_predictions.csv",
    )
    args = parser.parse_args()
    export_test_clone_data(args.pred_csv)
